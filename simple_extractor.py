...
def save_label_images(parsing_result, output_dir, img_name, labels):
    for label_index, label_name in enumerate(labels):
        label_mask = (parsing_result == label_index).astype(np.uint8) * 255
        label_img = Image.fromarray(label_mask)
        label_img.save(
            os.path.join(output_dir, f"{img_name[:-4]}-tag-{label_name}.png")
        )
...
def main():
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(dataloader)):
            ...
            # Save individual label images
            save_label_images(parsing_result, args.output_dir, img_name, labels)

            # Save the overall parsing result (optional)
